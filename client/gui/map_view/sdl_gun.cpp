...
SDLGun::SDLGun(SdlWindow& w):
        window(w),
        primaryTexture(std::string(GameConfig::Paths::PRIMARY_GUN_PATH), window, 42, 42),
        secondaryTexture(std::string(GameConfig::Paths::SECONDARY_GUN_PATH), window, 42, 42),
        meleeTexture(std::string(GameConfig::Paths::MELEE_GUN_PATH), window, 42, 42),
        bombTexture(std::string(GameConfig::Paths::BOMB_GUN_PATH), window, 42, 42) {}

// Render method
void SDLGun::render(ItemSlot itemSlot, Area& dest, float angle) {
    SDL_Rect src = {0, 0, 42, 42};
    switch (itemSlot) {
