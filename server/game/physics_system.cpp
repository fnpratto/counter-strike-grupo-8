...
PhysicsSystem::PhysicsSystem(const Map& map, const std::map<std::string, std::unique_ptr<Player>>& players) : 
        map(map), 
        players(players) {}

Vector2D PhysicsSystem::random_spawn_tt_pos() const {
    return map.random_spawn_tt_pos();
}

Vector2D PhysicsSystem::random_spawn_ct_pos() const {
    return map.random_spawn_ct_pos();
}
