...
protected:
    ...
    std::vector<T> merge(const std::vector<T>& a, const std::vector<T>& b) const {
        ...
        std::copy_if(b.begin(), b.end(), std::back_inserter(merged), [&merged](const T& v) {
            return std::find(merged.begin(), merged.end(), v) == merged.end();
        });
...
#define DEFINE_UPDATE(CLASS, ATTRS)                                               \
    class CLASS: public StateUpdate {                                             \
        ATTRS(X_DEFINE_MEMBER, M_DEFINE_MEMBER, U_DEFINE_MEMBER, V_DEFINE_MEMBER) \
                                                                                  \
    public:                                                                       \
        ATTRS(X_SETTER, M_SETTER, U_SETTER, V_SETTER)                             \
                                                                                  \
        CLASS merged(const CLASS& other) const {                                  \
            CLASS merged;                                                         \
                                                                                  \
            ATTRS(X_MERGER, M_MERGER, U_MERGER, V_MERGER)                         \
                                                                                  \
            return merged;                                                        \
        }                                                                         \
                                                                                  \
        ATTRS(X_CHANGED, M_CHANGED, U_CHANGED, V_CHANGED)                         \
                                                                                  \
        bool has_change() const override {                                        \
            ATTRS(X_HAS_CHANGE, M_HAS_CHANGE, U_HAS_CHANGE, V_HAS_CHANGE)         \
                                                                                  \
            return false;                                                         \
        }                                                                         \
                                                                                  \
        void clear() override { ATTRS(X_CLEAR, M_CLEAR, U_CLEAR, V_CLEAR) }       \
    };
