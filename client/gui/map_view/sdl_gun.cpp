...
SDLGun::SDLGun(SdlWindow& w): window(w) {
...
void SDLGun::render_gun(GunType gun_type, Vector2D pos, float angle) {
    auto it = gun_textures.find(gun_type);
    if (it != gun_textures.end()) {
        it->second->render(pos.get_x(), pos.get_y(), angle);
    } else {
        std::cerr << "Error: Invalid gun type passed to SDLGun::render_gun: "
                  << static_cast<int>(gun_type) << std::endl;
    }
}

void SDLGun::render_knife(Vector2D pos, float angle) {
    if (knife_texture) {
        knife_texture->render(pos.get_x(), pos.get_y(), angle);
    } else {
        std::cerr << "Error: Knife texture not loaded in SDLGun::render_knife." << std::endl;
    }
}

void SDLGun::render_bomb(Vector2D pos, float angle) {
    if (bomb_texture) {
        bomb_texture->render(pos.get_x(), pos.get_y(), angle);
    } else {
        std::cerr << "Error: Bomb texture not loaded in SDLGun::render_bomb." << std::endl;
    }
}
